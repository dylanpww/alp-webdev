...
@section('konten')
    ...
    <div class="container mt-4">
        <div class="row">
            <div class="col-md-6">
                <img src="{{ asset('storage/' . $room->image) }}" class="img-fluid rounded shadow-sm"
                    alt="{{ $room->type_name }}" style="width: 100%; height: 400px; object-fit: cover;">
            </div>
            <div class="col-md-6">
                <h2 class="fw-bold">{{ $room->type_name }}</h2>
                <h4 class="">Rp {{ number_format($room->price, 0, ',', '.') }} <small class="text-muted fs-6">/
                        night</small></h4>
                ...
                <div class="card bg-light border-0 p-3 mt-4">
                    <form action="{{ route('types.book') }}" method="GET">
                        ...
                        <div class="card-body">
                            ...
                            @if($alreadyReviewed)
                                <div class="alert alert-info">
                                    You have already submitted a review for this room type. Thank you!
                                </div>
                            @elseif ($canReview)
                                <form action="{{ route('ratings.store', $room->id) }}" method="POST">
                                    ...
                                    <button class="btn" style="background-color: #BA8B4E; border-color: #BA8B4E; ">Submit
                                        Review</button>
                                </form>
                            @else
                                <div class="alert alert-warning">
                                    You need to complete a stay in this room type to write a review.
                                </div>
